package com.example.olgac.myfirstapp;
...
public class SecondActivity extends AppCompatActivity {
    ...
    public void showRandomNumber () {
        ...
        int receiveCount = getIntent().getIntExtra(TOTAL_COUNT, 0);

        // Generate the random number
        Random random = new Random();
        int randomInt = 0;
        if (receiveCount>0) {
            randomInt = random.nextInt(receiveCount);
        }

        // Substitute the max value into the string resource
        // for the heading, and update the heading
        headingView.setText(getString(R.string.random_heading, receiveCount));
